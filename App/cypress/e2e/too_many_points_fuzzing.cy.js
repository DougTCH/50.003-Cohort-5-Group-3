...
describe('fuzzing test', () => {
  ...
  const runFuzzingTest = () => {
    ...
    const randomSubAmount = faker.datatype.number({ min: 0, max: randomAmount }).toString();
    const randomMoreAmount = (randomAmount + 10).toString();
    const validMembershipId = "BM1234";
    ...
    // Enter membership ID
    cy.get('input[placeholder="Insert your Membership ID here"]').type(validMembershipId);
    ...
    cy.get('input[placeholder="0"]').type(randomMoreAmount);
    cy.get('p.transaction-error').should('contain', 'Invalid Amount: Exceeds available points.')
/*
